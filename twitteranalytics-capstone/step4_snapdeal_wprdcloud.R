...
snapdeal_combined.corpus.ptd <- tm_map(snapdeal_combined.corpus, PlainTextDocument)
tdm<- TermDocumentMatrix(snapdeal_combined.corpus.ptd, control= list(wordLengths= c(1, Inf)))
...
ggplot(df, aes(reorder(term, freq),freq)) + theme_bw() + geom_bar(stat = "identity")  + coord_flip() +labs(list(title="Term Frequency Chart for Snapdeal", x="Terms", y="Term Counts"))+theme(text = element_text(size=10),axis.text.x = element_text(angle=90, vjust=1)) 
