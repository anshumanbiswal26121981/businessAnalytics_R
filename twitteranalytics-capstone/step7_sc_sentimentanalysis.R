...
scSentimentDataFrame = data.frame(text=sc_combined.df$text,emotion=sctweetsemotion, polarity=scPol, stringsAsFactors=FALSE)
scSentimentDataFrame%>%filter(!emotion %in% c("unknown")) ->sc.filter.df

sc.filter.df = within(sc.filter.df, emotion <-factor(emotion, levels=names(sort(table(emotion),decreasing=TRUE))))
...
getWordCloud(scSentimentDataFrame,rem.some.punct(sc_combined.df$text),sctweetsemotion)
