...
library(sentiment)
snapdealtweetsemo = classify_emotion(snapdeal_combined.df$text,algorithm="bayes", prior=1.0)
...
snapdealTweetsClassPol = classify_polarity(snapdeal_combined.df$text,algorithm="bayes")
head(snapdealTweetsClassPol,20)
snapdealPol = snapdealTweetsClassPol[,4]

snapdealSentimentDataFrame = data.frame(text=snapdeal_combined.df$text,emotion=snapdealemotion, polarity=snapdealPol, stringsAsFactors=FALSE)
snapdealSentimentDataFrame%>%filter(!emotion %in% c("unknown")) ->snapdeal.filter.df

snapdeal.filter.df = within(snapdeal.filter.df, emotion <-factor(emotion, levels=names(sort(table(emotion),decreasing=TRUE))))
plotSentiments1(snapdeal.filter.df, 'Sentiment Analysis of Tweets on Twitter about snapdeal')
write.csv(snapdeal.filter.df,file = "snapdeal_filter_sentiment_by_emotion.csv")

#snapdeal wordcloud with emotions
...
getWordCloud(snapdealSentimentDataFrame,rem.some.punct(snapdeal_combined.df$text),snapdealemotion)
